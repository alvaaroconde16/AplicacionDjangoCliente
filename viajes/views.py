...
def reservasMejoradas_lista_api(request):
    ...
    response = requests.get('http://127.0.0.1:8000/api/v1/reservasMejoradas', headers=headers)
    reservas = response.json()
...
def alojamientosMejorados_lista_api(request):
    ...
    response = requests.get('http://127.0.0.1:8000/api/v1/alojamientosMejorados', headers=headers)
...
def transportesMejorados_lista_api(request):
    headers = {'Authorization': 'Bearer ' + env('OAUTH2_ACCESS_TOKEN_ADMIN')} 
    response = requests.get('http://127.0.0.1:8000/api/v1/transportesMejorados', headers=headers)
